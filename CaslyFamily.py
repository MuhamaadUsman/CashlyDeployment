...
tables = pd.read_html(
    "https://www.feanalytics.com/Table.aspx?SavedResultsID=e33df1a2-7b9a-ee11-b204-002248818b97&xl=1&UserID=B2E540A8-D474-4C56-9088-1F155612F7FE&xlRefreshData=1")
...
def forecast_next_values(model, last_date, num_values=60):
    ...
    forecast_data = pd.DataFrame(
        index=pd.date_range(start=last_date + pd.Timedelta(days=1), periods=num_values, freq='M'))
...
def calculate_discrete_dataframe(df):
    ...
    Discrete = Discrete.T.reset_index().rename(
        columns={'index': 'Discrete Return', 0: 'Year1', 1: 'Year2', 2: 'Year3', 3: 'Year4', 4: 'Year5'})
...
def calculate_unit_dataframe(df):
    ...
    for i in range(df.shape[1]):
        ...
        # Transpose, reset index, and rename columns
    Unit = Unit.T.reset_index().rename(
        columns={'index': 'Unit', 0: 'Year1', 1: 'Year2', 2: 'Year3', 3: 'Year4', 4: 'Year5'})
...
def optimize_weights(df, target_objective, tolerance):
    ...
    for year in df.columns:
        ...
        m.addConstr(
            gp.quicksum(weights[i] * df[year][i] for i in range(num_total_assets)) >= target_objective - tolerance,
...
asset_names = ['Alternatives', 'Alternatives', 'Cash and Term Deposits', 'Cash and Term Deposits', 'Fixed Income',
               'Commodities', 'International Securities', 'International Securities', 'Australian Securities',
               'Australian Securities', 'Australian Securities', 'Australian Securities', 'International Securities',
               'International Securities', 'International Securities', 'International Securities',
               'International Securities', 'Infrastructure', 'International Securities', 'Private Equity',
               'Fixed Income', 'Fixed Income', 'Fixed Income', 'Fixed Income', 'Fixed Income', 'Fixed Income',
               'Fixed Income', 'Alternatives', 'Property', 'Property', 'Property']
...
df_historical_camulative = calculate_cumulative_returns(calculate_discrete_dataframe(data),
                                                        calculate_unit_dataframe(data))
...
app.layout = html.Div([  # Site Header
        ...
        html.Div([
                ...
                html.Ul([
                    ...
                    html.Li([
                        ...
                        html.Ul([
                            ...
                            html.Li(html.A("Global Product Range",
                                           href="https://cashelfo.com/wealth/global-product-range/")),
                            html.Li(html.A("Investment Platforms",
                                           href="https://cashelfo.com/wealth/investment-platforms/")),
                    ...
                    html.Li(html.A("Client Login",
                                   href="https://portal.xplorewealthservices.com.au/linearBpms/loginCashelHouse.jsp",
            ...
            html.Div([
                ...
                html.Div([
                    html.Ul([
                        html.Li(html.A("Home", href="https://cashelfo.com/")),
                        html.Li(html.A("Wealth Services", href="https://cashelfo.com/wealth/plan/",
                                       className="wealth-services")),
                        html.Li(html.A("Family Office", href="https://cashelfo.com/our-services/",
                                       className="family-office")),
                ...
                html.Div([
                    ...
                    html.Ul([
                        ...
                        html.Li(
                            html.A("Community", href="https://cashelfo.com/about-us/community/", id="menu-item-911")),
                    ...
                    html.Ul([
                        ...
                        html.Li(
                            html.A("Global Partnerships", href="https://cashelfo.com/about-us/global-partnerships/")),
                    ...
                    html.Ul([
                        ...
                        html.Li(html.A("Investment Platforms", href="https://cashelfo.com/wealth/investment-platforms/",
                                       id="menu-item-818")),
                        html.Li(html.A("Join Cashel Family Office", href="https://cashelfo.com/join-cashelfo/",
                                       id="menu-item-5154")),
    ...
    dcc.Graph(
        ...
        figure={
            'data': [
                {'x': correlate_table(data)['Correlation Number'], 'y': correlate_table(data)['Sectors'], 'type': 'bar',
                 'orientation': 'h'}
            ],
            'layout': {
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    ...
                    html.Br(),
                    html.A(
                        html.Img(src="https://cashelfo.com/wp-content/uploads/2023/12/google_play_badge.png", alt=""),
                        href="https://play.google.com/store/apps/details?id=com.softobiz.casher_family_office&hl=en_US&gl=US",
                        target="_blank", rel="noopener",
                        style={"max-width": "128px !important", "max-height": "min-content"}),
    ...
    for idx, selected_column in enumerate(selected_columns):
        for forecast_method in selected_forecast_methods:
            ...
            if forecast_method == 'original':
                ...
                fig.add_trace(
                    px.line(data, x=data.index, y=selected_column, title=f'{selected_column} over time').update_traces(
                        line=dict(color=line_color)).data[0])
            elif forecast_method == 'lstm':
                # Assuming you have 'lstm_forecast' DataFrame
                forecast_data = lstm_forecast_data[selected_column]

                fig.add_trace(px.line(lstm_forecast_data, x=lstm_forecast_data.index, y=selected_column,
                                      title=f'LSTM Forecast: {selected_column} (RMSE: {2.3:.2f}, MAE: {1.4:.2f})').update_traces(
                    line=dict(color=line_color)).data[0])
                ...
                fig.add_trace(px.line(xgboost_forecast_data, x=xgboost_forecast_data.index, y=selected_column,
                                      title=f'XGBoost Forecast: {selected_column} (RMSE: {4:.2f}, MAE: {2:.2f})').update_traces(
                    line=dict(color=line_color)).data[0])
            elif forecast_method == 'sarima':
                # Assuming you have 'sarima_forecast' DataFrame
                forecast_data = sarima_forecast_data[selected_column]

                fig.add_trace(px.line(sarima_forecast_data, x=sarima_forecast_data.index, y=selected_column,
                                      title=f'SARIMA Forecast: {selected_column} (RMSE: {6:.2f}, MAE: {1:.2f})').update_traces(
                    line=dict(color=line_color)).data[0])
                ...
                fig.add_trace(px.line(fbprophet_forecast_data, x=fbprophet_forecast_data.index, y=selected_column,
                                      title=f'FBProphet Forecast: {selected_column} (RMSE: {7:.2f}, MAE: {4.53:.2f})').update_traces(
                    line=dict(color=line_color)).data[0])
...
def update_output(n_clicks, selected_df, objective, tolerance):
    if n_clicks > 0:
        ...
        line_chart.add_trace(
            go.Scatter(x=['Year1', 'Year2', 'Year3', 'Year4', 'Year5'], y=set_risk_values, mode='lines',
                       name='Risk Values'))
...
def optimize_button_click(n_clicks, data_table_data, min_weight, max_weight, total_investment, volitility):
    ...
    historical_optimize_model, historical_risk, p, sr, spr = pre_optimize_weights(historical[historical.columns[1:6]],
    ...
    return optimal_columns, optimal_model.to_dict('records'), total_table_columns, total_table.to_dict(
        'records'), discrete_fig, cumulative_fig, portfolio_fig, Hdiscrete_fig, Hcumulative_fig, Hportfolio_fig, forecast_volatility, historical_volatility, {
        'data': [trace_simulated, trace_optimal], 'layout': layout}
